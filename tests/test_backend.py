...
class TestScores:
    ...
    def test_submit_score(self, authed_client):
        resp = client.post("/api/scores", json={"score": 10000}, headers=authed_client)
        assert resp.status_code == 201, resp.text

    def test_submit_score_unauthenticated(self):
        resp = client.post("/api/scores", json={"score": 10000})
        assert resp.status_code == 401

    def test_submit_invalid_score(self, authed_client):
        resp = client.post("/api/scores", json={"score": -1}, headers=authed_client)
        assert resp.status_code == 422
        resp = client.post("/api/scores", json={"score": 1000000}, headers=authed_client)
        assert resp.status_code == 422
