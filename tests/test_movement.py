"""
test_movement.py — Testes do buffer de input

Valida:
  - O buffer de direção consome comandos no centro do tile
  - Meias-voltas são executadas instantaneamente
  - Direções bloqueadas são ignoradas
  - O early snap funciona quando próximo ao centro

NOTA: Estes testes usam Node.js (jsdom + simulação) para
testar a lógica do Entity sem um navegador real.
Execute com: node tests/test_movement.js
"""

# ─── Esta seção contém um script Node.js de teste ─────────
# Execute: node tests/test_movement.js

MOVEMENT_TEST_SCRIPT = r"""
...
function test_blocked_direction_clears_buffer() {
  ...
  WALL_MAP[10][11] = TILE.WALL; // bloqueia tile 10,11 (down do tile 10,10)
  WALL_MAP[9][10] = TILE.WALL;  // bloqueia tile 9,10 (up)
  ...
  e.targetX = 11 * TS + TS / 2;
  e.targetY = 10 * TS + TS / 2;
  e.px = 11 * TS + TS / 2 - 1; // praticamente no centro
  e.py = 10 * TS + TS / 2;

  // Tenta virar para left (que está bloqueado na posição atual)
  e.setBufferedDir('up', WALL_MAP, false);
  // Não pode ir pra cima (tile 9,10 bloqueado)
  // buffer deve ser armazenado, depois descartado
  // O early snap tenta, mas falha porque tile (10+0, 10-1) = (10,9) está bloqueado
  // O buffer fica armazenado até o step() no centro
  // Ao chegar no centro, _applyBuffer tenta e falha
  // Vamos simular chegando ao centro
...
if __name__ == '__main__':
    print(MOVEMENT_TEST_SCRIPT)
    print("\n\nCopie este script e execute: node tests/test_movement.js")
    print("Para executar os testes do backend: source venv/bin/activate && pytest tests/test_backend.py -v")
